...
![variation regression](variation_regression.png)
$$p(t|w) = \prod_{n=1}^N N(t_n|w^T\phi_n, \beta^{-1})$$
$$p(w|\alpha) = N(w|0, \alpha^{-1}I)$$
$$p(\alpha) = Gamma(\alpha | a_0, b_0)$$
$$p(t,w,\alpha) = p(t|w) p(w|\alpha) p(\alpha)$$

### 10.3.1 变分分布

第一个目标是找到$p(a,w|t)$的近似。这里假设后验概率$q(w,\alpha) = q(w)q(\alpha)$通过变分可以得到:
...
### 10.3.2 预测分布
使用高斯变分后验概率可以得到
$$p(t|x,t) = \int N(t|w^T\phi(x),\beta^{-1}) N(w|m_N, S_N) dw$$
$$p(t|x,t) = N(t|m_N^T\phi(x),\sigma^{2}(x))dw$$
$$\sigma^2(x) = \beta^{-1} + \phi(x)^TS_N\phi(x)$$

### 10.3.3 下界
L表示模型的对数边缘似然函 数$\ln p(t|M)$的下界。
$$L = E[p(\alpha,w,t)] - E[q(w,\alpha)]$$

![model selection](model_selection.png)

在这个例子中下界最高的是M=3时，与之前多项式回归时结论的一致。

## 10.4 指数簇分布
对于本书中讨论的许多模型来说，完整数据是服从指数族分布的。然而，通常这对于观测数据的边缘似然函数来说是不成立的。 对指数家族通过变分可以得到：
$$q^*(z) = h(x_n,z_n)g(E_{\lambda}) \exp\{E[\lambda^T]u(x_n,z_n)\} $$
$$q^*(\lambda) = f(v_n,x_n)g(\lambda)^T \exp\{v_n\lambda^T x_n\} $$

其中潜在变量(记作Z)是分散的，它的数量随着数据集规模的增大而增大。参数(记作$\theta$)是聚集的，它的数量固定，与数据集的规模无关。
...
## 10.5 局部变分方法
前面讨论的变分框架可以被看做“全局”方法。之所以这样说，是因为它直接寻找所有随机变量上的完整的后验概率分布的近似。另一种“局部”的方法涉及到寻找模型中的单独的变量或者变量组上定义的函数的界限。
`略`
